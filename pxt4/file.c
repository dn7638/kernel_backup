...
static unsigned long get_idx_fn(void *elem)
{
return current->cpu;
}

static const char * get_name_fn(void *elem)
{
return "cpu";
}

static void print_fn(unsigned long index,
const char *name,
unsigned long long count,
int percentage)
{
printk("%s[%ld] called pxt4_file_write_iter %lld times (%d%%)\n",name, index , count, percentage);
}
...
DEFINE_DS_MONITORING(thread_dm, get_idx_fn, get_name_fn, print_fn);
...
//getrawmonotonic(&myclock[1]);
//calclock(myclock, &file_write_iter_time, &file_write_iter_count);
//printk("cpu[%u] pxt4_file_write_iter()", current->cpu);
